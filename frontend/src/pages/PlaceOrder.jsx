...
const PlaceOrder = () => {
  ...
  return(
    ...
    <form onSubmit={onSubmitHandler} className='flex flex-col sm:flex-row justify-between gap-4 pt-5 sm:pt-10 min-h-[80vh] border-t'>
      

      {/* -------------------------left side---------------------------- */}
      <div className='flex flex-col gap-3 w-full sm:max-w-[400px]'>
        <div className='text-lg sm:text-xl my-2'>
      ...
      <div className='mt-8'>
        ...
        <div className='mt-12'>
          ...
          <div className='flex gap-3 flex-col lg:flex-row'>
            <div onClick={()=>setMethod('stripe')} className='flex items-center gap-3 p-2 border px-3 cursor-pointer'>
              <p className={`min-w-3.5 h-3.5 border rounded-full ${method === 'stripe' ? 'bg-green-500' : ''} `}></p>
              <img className='h-5 mx-4' src={asset.stripe_logo} alt="" />
            </div>

            {/* <div onClick={()=>setMethod('razorpay')} className='flex items-center gap-3 p-2 border px-3 cursor-pointer'>
              <p className={`min-w-3.5 h-3.5 border rounded-full ${method === 'razorpay' ? 'bg-green-500' : ''}`}></p>
              <img className='h-5 mx-4' src={asset.razorpay_logo} alt="" />
            </div> */}

            <div onClick={()=>setMethod('cod')}  className='flex items-center gap-3 p-2 border px-3 cursor-pointer'>
